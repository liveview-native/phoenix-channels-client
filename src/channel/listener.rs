...
impl Listener {
            ...
            let next_state = match current_state {
                ...
                State::Joined(mut joined) => {
                    tokio::select! {
                        ...
                        left_result = &mut joined.left_receiver => self.left_result_received(joined, left_result),
                        Some(subscription_command) = self.subscription_command_receiver.recv() => {
                            self.update_subscriptions(subscription_command).await;
                        ...
                        broadcast_result = joined.broadcast_receiver.recv() => self.broadcast_result_received(joined, broadcast_result).await?,
                        else => break Ok(())
                    }
                }
                State::Leaving {
                    ref mut socket_left_receiver,
                    ..
                } => tokio::select! {
                    left_result_result = socket_left_receiver => self.left_result_result_received(current_state, left_result_result).await?,
                    Some(subscription_command) = self.subscription_command_receiver.recv() => {
                        self.update_subscriptions(subscription_command).await;
    ...
    async fn join(
        ...
        created_at: Instant,
        timeout: Duration,
        channel_joined_sender: oneshot::Sender<Result<(), channel::JoinError>>,
    ) -> Result<State, ShutdownError> {
        match state {
    ...
    fn joined_result_received(
        ...
        Self::send_to_channel_senders(channel_joined_senders, channel_joined_result);
    ...
    async fn socket_join(
        ...
        created_at: Instant,
        rejoin: Rejoin,
        channel_joined_senders: Vec<oneshot::Sender<Result<(), channel::JoinError>>>,
    ) -> Result<State, ShutdownError> {
        match self
            .socket
            .join(
    ...
    fn socket_shutdown(state: State) -> ShutdownError {
        match state {
            ...
            State::Joining(Joining {
                ...
                Self::send_to_channel_senders(
                    channel_joined_senders,
                    Err(channel::JoinError::Shutdown),
                );
            }
            State::Leaving {
                channel_left_senders,
                ...
                Self::send_to_channel_senders(channel_left_senders, Err(LeaveError::SocketShutdown))
            }
        }
    ...
    async fn leave(
        ...
        match state {
            ...
            State::Joining { .. } => match self.socket_leave(state, left_sender).await {
                (
                    State::Joining(Joining {
                        channel_joined_senders,
                        ..
            ...
            State::Joined { .. } => match self.socket_leave(state, left_sender).await {
                (_, Some(next_state)) => next_state,
                (unchanged_state, None) => unchanged_state,
            },
            State::WaitingToRejoin { .. } => {
    ...
    async fn left_result_result_received(
        ...
        match left_result_result {
            Ok(left_result) => {
                ...
                {
                    Self::send_to_channel_senders(channel_left_senders, left_result);
                }
...
pub(super) enum StateCommand {
    Join {
        ...
        timeout: Duration,
        joined_sender: oneshot::Sender<Result<(), channel::JoinError>>,
    },
    Leave {
        left_sender: oneshot::Sender<Result<(), LeaveError>>,
    },
...
struct Joined {
    ...
    left_receiver: oneshot::Receiver<Left>,
    join_timeout: Duration,
}
impl Joined {
