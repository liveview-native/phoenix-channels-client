...
impl Connected {
    ...
    fn send_disconnected(&mut self, left: Left) {
        ...
        for (topic, mut joined_channel_txs_by_join_reference) in
            self.joined_channel_txs_by_join_reference_by_topic.drain()
        {
            debug!("Sending {:?} to topic {:#?} channels", left, topic);

            for (join_reference, JoinedChannelSenders { left: left_tx, .. }) in
                joined_channel_txs_by_join_reference.drain()
            {
                debug!(
                ...
                left_tx.send(left).ok();
            }
        }

        for (topic, mut reply_tx_by_reference_by_join_reference) in self
            .reply_tx_by_reference_by_join_reference_by_topic
            .drain()
        {
            debug!(
