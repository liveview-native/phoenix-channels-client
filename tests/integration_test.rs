...
use phoenix_channels_client::{
    ...
    PresencesLeave, Socket, SocketStatus, Topic, WebSocketError, JSON,
};
    ...
    let (bob_join_reference, bob_online_at) = loop {
        ...
        match key.as_str() {
            ...
            "sockets:alice" => {
                ...
                let meta = &metas[0];
                assert_eq!(first_alice_join_reference, meta.join_reference.to_owned());
                assert_eq!(first_alice_online_at, &meta.others["online_at"]);
            }
            // Bob sees Bob Join
            "sockets:bob" => {
                assert_eq!(current, None);
                ...
                assert_eq!(metas.len(), 1);
                let meta = &metas[0];
                let bob_join_reference = meta.join_reference.to_owned();
                assert_ne!(bob_join_reference, first_alice_join_reference);
                let bob_online_at = meta.others["online_at"].clone();
